...
Do not turn every noun into a path or model styling, architecture, database schema, or implementation technology. Before asking a Stage 1 clarification, apply a scope-delta test to its proposed answers. Ask exactly one concise question per turn only when the input lacks enough application intent to derive a meaningful package name, or explicitly requested behavior has materially different plausible meanings and no coherent minimal interpretation is available without a user decision. Every answer must resolve the stated behavior without adding an unrequested user action, state transition, data item, external effect, lifecycle or stopping rule, or business policy. If an answer would add such behavior, treat the matter as an optional extension: do not ask it, do not add it through an assumption, and let closed scope prevail. A domain convention or common lifecycle operation is not explicit intent. An unspecified lifecycle or stopping condition is nonblocking whenever requested behavior remains coherent without it; repeated behavior then continues while its explicitly stated eligibility condition remains true. Stage 2 may later propose the extension. For a permitted question, wait for the answer, mark the choice `user-confirmed`, and never bundle it with another question. For other unstated details needed to interpret requested behavior, use the smallest coherent interpretation, mark it `generator-selected`, and record it as specified below.
...
Apply the framework input-path naming convention while introducing paths. Give each coherent family of transient values supplied directly by a user a role-specific root containing `Input`, normally `<Concept>Input`; do not apply the convention to persisted domain paths, bindings, environment-provided values, or unrelated occurrence and control paths.

### 2. Create a Compact Intermediate AOD-YAML

Create a temporary intermediate specification under the complete model summary. First derive controlled implicit declarations, concept-rooted and binding-relative constituents, prefixes, projections, and capability uses accurately. Then apply `aod_framework_package_profile.md` from longest path to shortest: every inferred or assumed path that is absent from all four AOD entry forms and is not a proper prefix of an entry already present or scheduled receives exactly one bare given-context entry. Do not add another bare entry for a path already explicit as a given entry, triggering path, or reaction target, or for a prefix covered by a longer entry. Thus an added `Task.Completed` also covers structural prefix `Task`; do not add `- Task` solely for that prefix. Never invent a path to avoid clarifying a materially ambiguous identity, binding, value source, or capability required by stated intent. An absent optional lifecycle or stopping condition is not such an ambiguity.
