...
- Preserve clear implicit declarations, prefixes, projections, and write-through relations; do not expand the specification merely to shorten the inference inventory.
...
- Choose given paths, standing definitions, and reaction targets by their model roles. Do not create trigger or initialization scaffolding around standing definitions.
...
- When a controlled-natural-language application concept intentionally reuses AOD path vocabulary, use the same spelling and capitalization, such as `new Task` for `Task`. Treat this as an interpretive naming convention, not formal type or instance syntax.
...
- Treat `persisted` in `all persisted Task` as a collection-source qualifier; it does not declare `Task.Persisted`. Use an explicit `.Persisted` path when a new structured value enters the persisted universe or when successful persistence of the current change is an observable causal prerequisite, not merely to restate existing persistence.
...
- Scope initial values to creation or another bound context. When several values describe one new instance, determine that instance as one structured valued binding. Phrase each intended constituent role and source clearly enough to infer its target-relative path; do not treat a mere source, condition, or dependency reference as a constituent. Use the binding's follow-on context for persistence.
...
- CreateButton.Clicked:
    - NewTask: new task for User.Current with title Input.Title and completed false
- NewTask:
    - NewTask.Persisted
```
