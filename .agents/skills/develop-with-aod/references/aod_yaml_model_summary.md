...
`D` must fit its target's role: one state or Boolean condition for a resultative path; one value, entity, collection, or binding for a valued path. Target direct state or property changes directly. Define derived collections and eligibility as current-state determinations rather than imperative insertion or removal instructions. Keep input, newly bound, persisted, selected, and draft contexts distinct; introduce a draft boundary only when changes must remain provisional until an explicit save-like attainment. Avoid `Prepared`, `Ready`, or `Recorded` helpers that only bundle properties or manufacture sequence; retain them for genuine independent conditions, bindings, acknowledgments, or capabilities.
