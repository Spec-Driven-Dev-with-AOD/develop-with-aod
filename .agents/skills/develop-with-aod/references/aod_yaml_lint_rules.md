...
Determine environment-contract coverage from required attainment semantics, not target form alone. A determined Boolean target requests establishment of the represented value and does not by itself request an effect. When either a bare or determined target depends on observation, persistence, or an environment-backed effect, require explicit contract coverage. For every capability target, identify the input paths it requires and verify that they are resolved on demand within the target attempt rather than turned into preceding reaction steps. If any required input cannot be resolved, the target remains unattained, creates no occurrence, and activates no follow-on reaction.
...
After completing the semantic inventory, compare every inferred or assumed AOD path with the paths that occur in the four AOD entry forms. A path is exactly explicit when it occurs as a bare or determined given-context entry, triggering path `Q`, or bare or determined reaction target. A path is prefix-covered when it is a proper prefix of an exactly explicit longer path. A textual reference inside `D` is not by itself an entry. Evaluate prospective normalization additions from longest path to shortest so an added longer entry also covers its prefixes.

For every inventory path that is neither exactly explicit nor prefix-covered after prospective normalization, report one framework-profile `WARNING` with category `PROF001`. State that the path may be validly implicit under the AOD model but the package does not conform to `aod_framework_package_profile.md`. Give the inference basis, every material use site, the exact bare entry `- P` to add, and the most relevant existing group. Do not warn for a path already explicit in another entry form, for a proper prefix covered by a longer entry, request a duplicate bare entry, infer an ambiguous path merely to satisfy the profile, or mutate the input. Resolve material inference ambiguity under the model audit before recommending an entry.
...
- when originating intent is available, each authoring assumption is necessary for the smallest coherent interpretation of stated behavior and adds no unrequested user action, state transition, data item, external effect, lifecycle or stopping rule, or business policy; absence of optional functionality or a nonblocking lifecycle or stopping condition is not a coverage defect;
