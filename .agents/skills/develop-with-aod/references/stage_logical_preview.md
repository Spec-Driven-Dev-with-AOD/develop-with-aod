...
- When attempting a capability target, resolve its required inputs, including applicable standing definitions, on demand. If a required input cannot be resolved, leave the target unattained, create no occurrence, and enter no follow-on reaction context. Otherwise enter its follow-on reaction context only after browser-real success or a result explicitly permitted by the preview simulation policy. Do not turn the inputs into preceding reaction steps.
